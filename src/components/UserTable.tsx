...
const UserTable = () => {
  ...
  useEffect(() => {
    fetchApi("appUsers", apiEndpoints.users.getAllUsers);
    return () => {
      resetApi("appUsers");
    };
  }, [fetchApi, resetApi]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (user) => (
        <Badge
          variant="outline"
          className={cn("font-normal capitalize", getPlanColor(user.plan))}
        >
          {user.plan || "Basic"}
        </Badge>
      ),
    ...
    //   key: "lastActive",
    //   header: "Last Active",
    //   render: (user) => formatDateWithTime(user.lastActive),
    // },
    {
      key: "loginCount",
      header: "Files",
